...
//   reality/app/xr/js/src/webxrcontrollersthreejs/tablet/interaction-provider.js
...
const getElementAtPosition = (clientX: number, clientY: number) => (
...
const createInteractionProvider = () => {
  const pointers_: Pointer[] = []
  const pointerMap_: Record<number, Pointer> = {}

  const activeTouches_: Touch[] = []
  const activeTouchesByTarget_ = new Map<Element, Touch[]>()

  let pointerId_ = 1

  const startPointer = (position, targetOverride: Element | null = null) => {
  ...
  const endPointer = (identifier: number, opts: EndPointerOptions = {}) => {
    const pointer = pointerMap_[identifier]
...
type InteractionProvider = ReturnType<typeof createInteractionProvider>
